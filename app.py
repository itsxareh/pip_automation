            ...
            if 'Account No.' in df.columns:
                ...
                if hasattr(dataset_response, 'data') and dataset_response.data:
                    ...
                    monitoring_df['Stores'] = monitoring_df['Account Number'].map(
                        lambda acc_no: '' if account_data_map.get(acc_no, {}).get('Stores' in ['0', 0] else account_data_map.get(acc_no, {}).get('Stores', '')))
                    
                    monitoring_df['Cluster'] = monitoring_df['Account Number'].map(
                        lambda acc_no: '' if account_data_map.get(acc_no, {}).get('Cluster' in ['0', 0] else account_data_map.get(acc_no, {}).get('Cluster', '')))
